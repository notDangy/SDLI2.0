...
function IsRotClose(rot, rot2)
	return math.abs(rot2 - rot) < 1
end
...
Citizen.CreateThread(function()
	...
	while true do
		...
		Citizen.Wait(5000)
		local pos = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 0.0, 0.0)

		for k,v in ipairs(Config.Doors) do

			if GetDistanceBetweenCoords(pos, v["Pos"].x, v["Pos"].y, v["Pos"].z, true) < 100.0 then

				local st = StartShapeTestBox(v["Pos"], 1.0, 1.0, 1.0, 0.0, 0.0, 0.0, true, 16)
				local value, hit, coords, surface, ent = GetShapeTestResult(st)

				if (GetEntityModel(ent) == v["Door"]) then

					if (not IsRotClose(GetEntityRotation(ent).z, v["Open"])) then

						FreezeEntityPosition(ent, true)
						SetEntityRotation(ent, 0.0, 0.0, v["Open"], 2, true)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
    Citizen.InvokeNative(0x4CC5F2FC1332577F,1058184710) --Carte Skill e Munizioni
    end
end)
...
RequestImap(-2082201137)  --Blackwater Ground Town Hall (Kira tolto)
--RequestImap(1343343014)  --Blackwater Town Hall Addons Construction (Kira tolto)
--RequestImap(739412171)  -- Two Boards in front of city hall (Goes with Town Hall Construction)
RequestImap(-5339556)  --Bank Under Construction (Kira tolto banca, cantiere)
...
--RequestImap(306939642) --TENDA BUGGATA 
...
--RequestImap(-537757838) -- Botola CASA 25
...
--RequestImap(-406964748) -- CARTELLO "CHIUSO" RHODES SALOON
RequestImap(481636996)
RequestImap(-1671953459)
--RequestImap(910941329) --BOX BIANCO SALOON RHODES
...
--RequestImap(-342806042) --INCIDENTE TRENO SAINT DENIS
...
--RequestImap(1184975829) -- BLOCCO MASSI ACCAMPAMENTO MEXICO
