...
Citizen.CreateThread(function()
	while true do
        ...
        if betweencoords < 1.0 then
            DrawTxt("Premi G per leggere il ricettario", 0.50, 0.90, 0.7, 0.7, true, 255, 255, 255, 255, true)
            if IsControlJustPressed(2, 0x760A9C6F) then
                TriggerServerEvent("ricettariosaloon:checkgroupsv")
            end
        end
	end
end)
...
RegisterNetEvent('ricettariosaloon:checkgroupcl')
AddEventHandler('ricettariosaloon:checkgroupcl', function() 
    ToggleBacheca()
end)