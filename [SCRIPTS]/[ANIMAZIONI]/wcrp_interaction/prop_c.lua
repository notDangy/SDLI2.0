...
function FPrompt(text, button, hold)
    Citizen.CreateThread(function()
        ...
        PropPrompt=nil
        local str = text or "Metti Via" --Put Away
...
--PocketWatchOro
RegisterNetEvent('prop:orologio')
AddEventHandler('prop:orologio', function() 
    RequestAnimDict('mech_inventory@item@pocketwatch@unarmed@base')
	FPrompt()
    while (not HasAnimDictLoaded('mech_inventory@item@pocketwatch@unarmed@base')) do
		Citizen.Wait(300)
	...
	--ExecuteCommand('close')
	prop_name = 's_inv_pocketwatch06x'
	local ped = PlayerPedId()
	local x,y,z = table.unpack(GetEntityCoords(ped, true))
	local prop = CreateObject(GetHashKey(prop_name), x, y, z + 0.2, true, true, true)
	local boneIndex = GetEntityBoneIndexByName(ped, "SKEL_R_Hand")
...
AddEventHandler('prop:pipe', function() 
   ...
   -- AttachEntityToEntity(pipe, ped, righthand, 0.005, -0.045, 0.0, -170.0, 10.0, -20.0, true, true, false, true, 1, true)
   AttachEntityToEntity(pipe, ped, righthand, 0.150, -0.045, 0.0, -170.0, 30.0, -50.0, true, true, false, true, 1, true)
...
AddEventHandler('prop:chewingtobacco', function()
    ...
    FPrompt("Smetti di masticare", 0x3B24C470, false)
    LMPrompt("Fai Qualcosa", 0x07B8BEAF, false)
    EPrompt("Canbia Posizione", 0xD51B784F, false)
    --ExecuteCommand('close')
    local ped = PlayerPedId()
    local x,y,z = table.unpack(GetEntityCoords(ped, true))
    local righthand = GetEntityBoneIndexByName(ped, "SKEL_R_Finger13")
