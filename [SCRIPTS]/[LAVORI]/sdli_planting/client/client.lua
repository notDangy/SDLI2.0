...
AddEventHandler('poke_planting:planto1', function(hash1, hash2, hash3, seed)
    ...
    for k,v in pairs(Config.Locations) do
        ...
        if GetDistanceBetweenCoords(v.x, v.y, v.z, pos.x, pos.y, pos.z, true) < 30.0 and canPlant then
            local distanced = true
            if (not v.illegal and seed~="cornseed") or (v.illegal and seed=="cornseed") then
                if myPlants then
                    for _,info in pairs(myPlants) do 
                        local dist = GetDistanceBetweenCoords(pos.x, pos.y, pos.z, info['x'], info['y'], info['z'], true)
                        if dist < 2 then
                            distanced = false
                            break
                        end
                ...
                if distanced then
                    canPlant = false
                    TriggerServerEvent("poke_planting:removeSeed", seed)
...
RegisterNetEvent("canwater:wait")
AddEventHandler("canwater:wait", function()
    Citizen.CreateThread(function() 
        Wait(5000)
        canWater = true
    end)
				...
				if GetDistanceBetweenCoords(v.x, v.y, v.z, pos.x, pos.y, pos.z, true) < 7.0 then
					...
					if v.prompt then
						if Citizen.InvokeNative(0x91AEF906BCA88877, 0, 0xCEFD9220) then
                            ...
                            canPlant = false
                            harvestPlant(key)
                            TriggerEvent('dangy_stress:modify', 2.5)
                            canPlant = true
...
Citizen.CreateThread(function()
    for _, st in pairs(Config.Locations) do
        if not st.illegal then
            local blip = N_0x554d9d53f696d002(1664425300, st.x, st.y, st.z)
            SetBlipSprite(blip, -675651933, 1)
            SetBlipScale(blip, 0.2)
            Citizen.InvokeNative(0x9CB1A1623062F402, blip, "Campo")
        end
    end  
end)
