...
local titleHeight = 0.075
local titleYOffset = 0.010  -- 0.010
local titleScale = 0.90
...
local buttonTextXOffset = 0.003  -- 0.003
...
local function drawTitle()
	if menus[currentMenu] then
		...
		local y = menus[currentMenu].y + titleHeight / 2.4
		...
		if HasStreamedTextureDictLoaded("menu_textures") then
			...
			DrawSprite("generic_textures", "menu_header_1a", x, y, menus[currentMenu].width + .02, titleHeight, 0.0, 255, 255, 255, 255, 0)
		end
...
local function drawSubTitle()
	if menus[currentMenu] then
		...
		if HasStreamedTextureDictLoaded("generic_textures") then
			...
			DrawSprite("generic_textures", "selection_box_bg_1d", x, y - 0.0015, menus[currentMenu].width + 0.02 , buttonHeight, 0.0, 0, 0, 0, 255, 0)
		    --DrawSprite("menu_textures", "translate_bg_1a", x, y+0.02, 0.26, 0.35, 0.8, 000, 2, 2, 150, 1)
		end
...
local function drawButton(text, subText)
	...
	if multiplier then
		...
		if menus[currentMenu].currentOption == optionCount then
			if HasStreamedTextureDictLoaded("generic_textures") then
				SetScriptGfxDrawOrder(1)
				DrawSprite("generic_textures", "selection_box_bg_1d", x, y - 0.002, menus[currentMenu].width+ 0.02, buttonHeight, 0.0, 255, 255, 255, 255, 0)
			end
		else
			if HasStreamedTextureDictLoaded("generic_textures") then
				SetScriptGfxDrawOrder(1)
				DrawSprite("generic_textures", "selection_box_bg_1d", x, y - 0.002, menus[currentMenu].width+ 0.02, buttonHeight, 0.0, 0, 0, 0, 255, 0)
			end
		end
...
function WarMenu.CreateMenu(id, title)
	...
	menus[id].x = 0.77  --0.02
	menus[id].y = 0.12

	menus[id].width = 0.21

	menus[id].currentOption = 1
	menus[id].maxOptionCount = 10  --4
